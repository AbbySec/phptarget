...
if(Yii::$app->session->hasFlash('advuploadsuccess')){
    echo \yii\bootstrap\Alert::widget([
        'options' => [
            'class' => 'bg-primary', //这里是提示框的class
        ],
        'body' => Yii::$app->getSession()->getFlash('advuploadsuccess'), //消息体
    ]);
}
?>
                    ...
                    <div class="container">
                        <div class="row">
                            ...
                            foreach (array_slice($data,0,4) as $key => $value){
                                echo WorkWidget::widget(['url'=>\yii\helpers\Url::toRoute(['advertisment/detail','id'=>$value['id']]),'content'=>strcut(HtmlPurifier::process($value['works_intro'])),'editor'=>\frontend\models\DisUser::find()->where(['id'=>$value['user_id']])->one()->username]);
                            }
