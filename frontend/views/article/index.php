...
if(Yii::$app->session->hasFlash('artuploadsuccess')){
    echo \yii\bootstrap\Alert::widget([
        'options' => [
            'class' => 'bg-primary', //这里是提示框的class
        ],
        'body' => Yii::$app->getSession()->getFlash('artuploadsuccess'), //消息体
    ]);
}
                        ...
                        <div class="row">
                            <?php
                                foreach (array_slice($data,0,4) as $key => $value){
                                    echo WorkWidget::widget(['url'=>\yii\helpers\Url::toRoute(['article/detail','id'=>$value['id']]),'content'=>strcut(HtmlPurifier::process(json_decode($value['works_intro'])[0])),'editor'=>\frontend\models\DisUser::find()->where(['id'=>$value['user_id']])->one()->username]);
                                }
