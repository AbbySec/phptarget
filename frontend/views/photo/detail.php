    ...
    <div class="ms-hero ms-hero-material">
        ...
        <div class="container">
            ...
            <div class="top-content">
                <div class="show-div1">
                    <h1 style="text-align: center;color: white"><?=def(\frontend\models\DisClass2::find()->where(['id'=>$model['works_class2_id']])->one()->name)?></h1>
                    <h4>作者：<?=def(\frontend\models\DisUser::find()->where(['id'=>$model['user_id']])->one()->username)?></h4>
                    <h4>点击量:  <?=def($model['comment_times']);?></h4>
                    <h4><?=def($model['works_intro'])?></h4>
                    <br>
                    <br>
                    <br>
                    <br>
                    ...
                    <ul class="show-ul">
                        <?php foreach ($work as $key => $value){ ?>
                            <li>
                                ...
                                <a href="<?php echo \yii\helpers\Url::toRoute(['article/detail','id'=>$value['id']]) ?>" id="show"><?=def(json_decode($value['works_intro'])[0])?></a>
                            </li>
                        <?php } ?>
                    </ul>
                </div>
