...
class InternetController extends \yii\web\Controller
{
    ...
    {
        $data = DisWorksInfo::find()->where(['works_class1_id' => '8', 'ispass' => '1'])->limit(16)->asArray()->all();
    ...
    public function actionDetail($id)
    {
        if (is_numeric($id))
        {
            ...
            $work = DisWorksInfo::find()->where(['works_class1_id' => '8'])->asArray()->limit(5)->all();
    ...
    public function actionPrint()
    {
        ...
        if (!$model)
        {
            ...
            return $this->redirect(['internet/upload']);
        }
        $this->layout = false;
    ...
    public function actionUpload()
    {
        ...
        if ($params)
        {
            ...
            $params['DisWorksInfo']['works_class1_id'] = '8';
            ...
            {
                \Yii::$app->session->setFlash('intuploadsuccess', '报名成功');
                //\Yii::$app->session->set('intobj',$model);
                //$this->redirect(['html5/print']);
            } else
            {
                \Yii::$app->session->setFlash('error', '报名失败，请重试');
            }
        } else
        {
            $model = new DisWorksInfo();
