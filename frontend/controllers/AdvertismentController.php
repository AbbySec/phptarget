...
class AdvertismentController extends \yii\web\Controller
{
    ...
    {
        $data = DisWorksInfo::find()->where(['works_class1_id' => '4', 'ispass' => '1'])->limit(16)->asArray()->all();
    ...
    public function actionPrint()
    {
        ...
        if (!$model)
        {
            ...
            return $this->redirect(['advertisment/upload']);
        }
        $this->layout = false;
    ...
    public function actionDetail($id)
    {
        if (is_numeric($id))
        {
            ...
            $work = DisWorksInfo::find()->where(['works_class1_id' => '4'])->asArray()->limit(5)->all();
    ...
    public function actionUpload()
    {
        ...
        if ($params)
        {
            ...
            $params['DisWorksInfo']['works_class1_id'] = '4';
            if ($model->load($params) && $model->validate() && $model->save())
            {
                \Yii::$app->session->setFlash('advuploadsuccess', '作品上传成功');
                //\Yii::$app->session->set('advobj',$model);
                $this->redirect(['advertisment/print']);
            } else
            {
