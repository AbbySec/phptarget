...
class IdeaController extends \yii\web\Controller
{
    ...
    {
        $data = DisWorksInfo::find()->where(['works_class1_id' => '5', 'ispass' => '1'])->limit(16)->asArray()->all();
    ...
    public function actionDetail($id)
    {
        if (is_numeric($id))
        {
            ...
            $work = DisWorksInfo::find()->where(['works_class1_id' => '5'])->asArray()->limit(5)->all();
    ...
    public function actionPrint()
    {
        ...
        if (!$model)
        {
            ...
            return $this->redirect(['idea/upload']);
        }
        $this->layout = false;
...
//            var_dump(json_encode($stuarr))
            ...
            $params['DisWorksInfo']['works_class1_id'] = '5';
            ...
            {
                \Yii::$app->session->setFlash('ideuploadsuccess', '作品上传成功');
                //\Yii::$app->session->set('ideaobj',$model);
                $this->redirect(['idea/print']);
            } else
            {
